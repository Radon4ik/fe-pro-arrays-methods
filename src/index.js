...
return goods.reduce((previousValue, currentValue) => {
 return Math.min(previousValue, currentValue.price);

  },Infinity);
};

const maxPriceReducer = () => {
  return goods.reduce((prev, curr)=> {
    return Math.max(prev, curr.price)
  },0);
};

const toMaxSorter = () => {
  return goods.sort((firstNum, secondNum) => {
    if(firstNum.price < secondNum.price) return 1;
    if(firstNum.price > secondNum.price) return -1;
    return 0;
  })
};
const toMinSorter = () => {
  return goods.sort((fNum, sNum) =>{
 if(fNum.price > sNum.price) return 1;
 if(fNum.price < sNum.price) return -1;
 return 0;
  });
};
